module suilend_pyth::accumulator {
    use std::vector::{Self};
    ...
    use suilend_pyth::merkle_tree::{Self};
    ...
    fun parse_price_feed_message(message_cur: &mut Cursor<u8>, clock: &Clock): PriceInfo {
        ...
        let price_info = price_info::new_price_info(
            ...
            price_feed::new(
                price_identifier,
                suilend_pyth::price::new(price, conf, expo, publish_time),
                suilend_pyth::price::new(ema_price, ema_conf, expo, publish_time),
            )
        );
        price_info
    }
